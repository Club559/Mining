...
namespace wServer.realm.commands
{
        ...
        public void Execute(Player player, string[] args)
        {
            ...
            foreach (var i in t.Assembly.GetTypes())
                if (t.IsAssignableFrom(i) && i != t)
                {
                    ...
                    if(instance.RequiredRank <= player.Client.Account.Rank)
    ...
    class ForgeListCommand : ICommand
       ...
       class ForgeCommand : ICommand
       {
           ...
           public void Execute(Player player, string[] args)
           {
               try
               {
                   ...
                   {
                       player.SendInfo("It costs " + (player.combs.Combo.Item2 / (player.HasSlot(3) && player.Inventory[3].ObjectType == 0x193e ? 2 : 1)).ToString() + " gold to forge these items. Are you sure?\nType /yes or /no");
                       player.Decision = 1;
                   }
               }
               catch
       ...
       class YesCommand : ICommand
       {
           ...
           public void Execute(Player player, string[] args)
           {
               if (player.Decision == 1)
               {
                   ...
                   {
                       if (player.combs.Combo.Item2 / (player.HasSlot(3) && player.Inventory[3].ObjectType == 0x193e ? 2 : 1) <= player.Credits)
                       {
                           short resultId;
                           if (XmlDatas.IdToType.TryGetValue(player.combs.Combo.Item1, out resultId))
                           {
                               ...
                               if (XmlDatas.ItemDescs.TryGetValue(resultId, out resultItem))
                               {
                                   ...
                                   player.Credits = player.Client.Account.Credits = new Database().UpdateCredit(player.Client.Account, -(player.combs.Combo.Item2 / (player.HasSlot(3) && player.Inventory[3].ObjectType == 0x193e ? 2 : 1)));
                       ...
                       {
                           player.SendInfo("Not enough gold to forge these items!");
                       }
                   }
               }
               player.Decision = 0;
       ...
       {
           public string Command { get { return "recipes"; } }
           public int RequiredRank { get { return 0; } }
