...
namespace wServer.logic.loot
{
    class LootBehavior : ConditionalBehavior
    {
        ...
        public void ShowBags(Random rand, IEnumerable<Item> loots, Player owner)
        {
            int bagType = 0;
        ...
        void ProcessSoulBags(Random rand, Tuple<Player, int>[] dat)
        {
            ...
            foreach (var i in items)
                if (i.Value.Count > 0)
                    if (!(Host is Wall)) ShowBags(rand, i.Value, i.Key); else ShowBagsW(rand, i.Value, i.Key, wOwner);
        }
